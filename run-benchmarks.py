...
def get_profiler(args):
    if args.profiler == "none":
        ...
        return NCUProfiler()
    elif args.profiler == "nvvp":
        return NVVPProfiler()
    raise Exception("Invalid profiler")
...
def bench_fpcompress(output_dir: str, profiler):
    ...
    for alg_type in ["speed", "ratio"]:
        for files, float_type in zip(
            ...
            compress_executable = f"./{float_type}_src/bin/{alg_type}-gpu-compress"
            ...
            for file in files:
                ...
                for i in range(1, args.number_sampling_runs + 1):
                    ...
                    bin_parameters = convert_list_to_str(
                        [compressed_file_path, decompressed_file_path, "y"]
                    )

                    profiler.benchmark_command(
                        decompress_executable + " " + " ".join(bin_parameters),
                        out=out,
                        append_signal=True,
                    )
